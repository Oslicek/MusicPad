...
public class FallbackAudioEncoder : IAudioEncoder
{
    ...
    /// <summary>
    /// Encodes to MP3. Requires FFmpeg - no fallback available.
    /// </summary>
    public async Task<bool> EncodeToMp3Async(float[] samples, int sampleRate, int channels, int bitrate, string outputPath)
    {
        // MP3 requires FFmpeg, no fallback
        return await _ffmpegEncoder.EncodeToMp3Async(samples, sampleRate, channels, bitrate, outputPath);
    }
